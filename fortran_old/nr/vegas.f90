SUBROUTINE vegas(region,func,init,ncall,itmx,nprn,tgral,sd,chi2a)
  ...
  REAL(WP), SAVE :: calls,dv2g,dxg,f,f2,f2b,fb, &
    rc,ti,tsi,wgt,xjac,xn,xnd,xo,harvest
  ...
  if (init <= 2) then
    ...
    ng=1
    if (mds /= 0) then
      ng=(ncall/2.0_wp+0.25_wp)**(1.0_wp/ndim)
      mds=1
      if ((2*ng-NDMX) >= 0) then
        mds=-1
        npg=ng/NDMX+1
...
200  format(/' input parameters for vegas:  ndim=',i3,'  ncall=',f10.0&
    /28x,'  it=',i5,'  itmx=',i5&
    /28x,'  nprn=',i3,'  alph=',f5.2/28x,'  mds=',i3,'   nd=',i4&
    /(30x,'xl(',i2,')= ',g11.4,' xu(',i2,')= ',g11.4))
201  format(/' iteration no.',I3,': ','integral =',g27.20,' +/- ',g9.2,&
    /' all iterations:   integral =',g27.20,' +/- ',g9.2,&
    ' chi**2/it''n =',g9.2)
